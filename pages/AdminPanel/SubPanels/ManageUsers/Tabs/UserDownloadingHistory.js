...
export default function UserDownloadingHistory(props) {
...
useEffect(()=>{
   ...
   if(Object.keys(props.selectedItem).length>0)
    setListOfUsers(props.selectedItem.userDownloadingHistor);
},[props.selectedItem])

  const handelDownloadCSV = () => {
    const title = props.selectedItem.designTitle + "_downloading_history";
    let rows = [];
    props.selectedItem.userDownloadingHistor.forEach((item) => {
      rows.push([
        item.downloadId,
        item.downloadItemType,
        item.downloadItemTitle,
        item.downloadDateAndTime,
      ]);
    }); 
                            ...
                            {listOfUsers.map((person, personIdx) => (
                              ...
                              >
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                  {person.downloadItemType}
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                  {person.downloadItemTitle}
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                  {person.downloadDateAndTime}
                                </td>
                              </tr>
                            ))}
                          </tbody>
