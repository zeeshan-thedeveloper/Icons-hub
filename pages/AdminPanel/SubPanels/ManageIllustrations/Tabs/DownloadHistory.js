...
function DownloadHistory(props) {
  ...
  //   downloadedIluustrationTitle: "Iluustration title",
  //   userName: "ABC",
  ...
  useEffect(() => {
    ...
    if (shouldListReload) {
      fetch("/api/AdminPanel_api/download_downloading_historyof_illustration_api", {
        method: "POST",
        body: JSON.stringify({}),
        headers: {
          ...
          if (data.responseCode === 1) {
            ...
            data.responsePayload.forEach((item, index) => {
              ...
              item.dowloadedIllustrations.forEach((illus, illusIndex) => {
                tempArray.push({
                  downloadId: illus.dowloadedIllustrationId,
                  userEmail: item.email,
                  downloadedIluustrationTitle: illus.illstrationTitle,
  ...
  const handelDownloadCSV = () => {
    ...
    listOfUsers.forEach((item) => {
      rows.push([
        ...
        item.userWebsite,
        item.userCountry,
        item.downloadedIluustrationTitle,
      ]);
    });
                ...
                <tbody>
                  {listOfUsers.map((person, personIdx) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {person.downloadedIluustrationTitle}
                      </td>
                    </tr>
                  ))}
                </tbody>
