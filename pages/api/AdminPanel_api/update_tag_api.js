...
async function handler(req, res) {
  if (req.method === "POST") {
    try {
      ...
      const db = client.db();
      const meta_data_collection = db.collection("meta_data_tags");
      const payload = {
        title: req.body.title,
        _id:req.body._id
      };

      const result = await meta_data_collection.updateOne({"_id": ObjectID(payload._id)},{"$set":{title:payload.title}},(error,response)=>{
        res.status(201).json({
            responseCode: 1,
